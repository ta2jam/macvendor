...
- Her yanıtta `X-API-Version: v1`, deploy edilen SemVer'i taşıyan `X-App-Version` ve `X-Request-Id` bulunur. Geçerli bir istemci request ID değeri güvenli biçimde doğrulanabiliyorsa korunur, aksi halde sunucu üretir.
- Resmî atama ile kürasyonlu iddia aynı nesnede birleştirilmez.
- Eşleşme yokluğu hata değildir: lookup ucu `200`, `matchStatus: "no_match"`, `assignment: null` ve release metadata döndürür.
...
- `mode=enriched|official`: varsayılan `enriched`. `enriched`, assignment, curatedMatches ve insights katmanlarını ayrı döndürür. Eski `mode=all`, geriye uyum için `enriched` alias'ı olarak kabul edilir. `official`, curatedMatches ve insights alanlarını boş dizi olarak döndürür.
- Başka parametre V1'de kabul edilmez; bilinmeyen parametre `400 UNSUPPORTED_PARAMETER` üretir.

### Başarılı yanıt

Full-MAC official assignment araması sabit 36, 28 ve 24-bit adaylarını bu
sırada indexed olarak dener. İlk bulunan aktif kayıt kazanır; MA-M `/28` bir
MA-L `/24` kaydını, MA-S veya IAB `/36` kaydı da örtüşen `/28` ve `/24`
kayıtlarını geçersiz kılmadan lookup sonucunda önceler. CID full-MAC lookup'a
katılmaz.
...
`matchStatus`, official assignment katmanı için `matched | no_match` olur.
`no_match` durumunda `assignment` zorunlu olarak `null` kalır; curatedMatches
ve insights yine ayrı ayrı bulunabilir. Adres alanı kaynakta yoksa,
yayımlanmasına izin verilmiyorsa veya bastırılmışsa `null` olur; alan kaldırılmaz.
...
| `matchStatus` | `matched \| no_match` | Hayır |
...
Tek istekte varsayılan `official` modunda 1–100, açıkça seçilen `enriched`
modunda 1–50 MAC kabul edilir:
`{"mode":"enriched","macs":["001122334455","02:AA:BB:CC:00:01"]}`.
`official` geriye uyumlu varsayılandır ve yalnız assignment ile release
metadata döndürür. `enriched`, her item'a ayrı `curatedMatches`,
`curatedMatchesTruncated`, `insights` ve `insightsTruncated` alanlarını ekler;
bu katmanlar assignment'ı değiştirmez.

Tüm değerler geçerli olmalıdır; tek bir invalid değer bütün isteği `400
INVALID_MAC` ile reddeder. Tekrarlanan girişler SQL input'unda deduplicate
edilir fakat çıktı sırası ve tekrarlar korunur. Quota maliyeti duplicate'ler
dahil gönderilen kayıt sayısına göre hesaplanır: official için her iki kayıt
başına 1 unit yukarı yuvarlanır; enriched için kayıt başına 1 unit. Yanıt
private, `no-store` olur.

Standart public quota istemci IP'si başına sabit 10 saniyelik pencerede 50 cost
unit'tir. Tekil lookup ve release uçları 1; organization uçları 2; evidence ve
correction işlemleri 5 unit harcar. Limit aşımında `429` ve saniye cinsinden
`Retry-After` döner.
...
`type` URI'si kalıcı problem türü dokümanına çözülür. `title` aynı problem türü için sabittir; `detail` occurrence'a özeldir ve istemci tarafından parse edilmez. JSON içindeki `status`, gerçek HTTP status koduyla aynı olmak zorundadır. `code`, `requestId`, `apiVersion` ve `appVersion` RFC 9457 extension alanlarıdır. Bütün problem yanıtları `private, no-store` kullanır.
